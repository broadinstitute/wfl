# Covid module

WorkFlow Launcher (WFL) implements the `covid` module to process workflows using the `Sarscov2IlluminaFull` pipeline. 

The `covid` module uses a model which includes a source, a sink and an executor. The source for the `covid` module at the time of this writing is either directly from the Terra Data Repository (`TerraDataRepo`), or a list of snapshots which are passed to WFL (`TDR Snapshots`). 

### API

The `covid` module implements the following _multimethod dispatchers_:

- create-workload!
- start-workload!
- add-workload!

It supports the following API endpoints:

| Verb | Endpoint                            | Description                                                              |
|------|-------------------------------------|--------------------------------------------------------------------------|
| GET  | `/api/v1/workload`                  | List all workloads, optionally filtering by uuid or project              |
| GET  | `/api/v1/workload?uuid={uuid}`      | List all workloads, optionally filtering by uuid or project              |
| GET  | `/api/v1/workload/{uuid}/workflows` | List all workflows for a specified workload uuid                         |
| POST | `/api/v1/create`                    | Create a new workload                                                    |
| POST | `/api/v1/start`                     | Start a workload                                                         |
| POST | `/api/v1/stop`                      | Stop a running workload                                                  |
| POST | `/api/v1/exec`                      | Create and start (execute) a workload                                    |

Different from the fixed workload types in which caller only needs to create a workload with a series of sample inputs and then simply start the workload, `covid` module requires the caller to manage the life cycle of a workload on their own in a multi-stage manner:

1. The caller needs to create a workload and specify the source, sink and executor. The source must be either a snapshot or a list of snapshots. If the former, then the type is `TDR Snapshots`. If the latter, than it is of type `TDRSnapshotListSource`. The executor must be of type `TerraExecutor`, and the sink must be of type `TerraWorkspaceSink`. If everything else passes verification, then the caller will receive a response which will contain the `uuid` of the workload.
   
2. Next, the caller needs to "start" the newly created workload, which will begin the analysis. 
   
3. WFL can, in addition, stop watching a workflow using the `stop` endpoint. This will not cancel analysis, but WFL will stop managing the workload.

To give more information, here are some example inputs to the above endpoints:

**GET /api/v1/workload**

==="Sample Request"

    ```
    curl 'http://localhost:8080/api/v1/workload' \
        -H 'Authorization: Bearer '$(gcloud auth print-access-token) \
        -H 'Accept: application/json'
    ```

=== "Sample Response"

    ```
    [ 
        {
            "watchers" : [ "tester@123.com" ],
            "labels" : [ "hornet:test", "project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy" ],
            "creator" : "ranthony@broadinstitute.org",
            "created" : "2021-06-15T13:25:07Z",
            "source" : {
            "dataset" : "4bb51d98-b4aa-4c72-b76a-1a96a2ee3057",
            "table" : "flowcells",
            "column" : "last_modified_date",
            "snapshotReaders" : [ "workflow-launcher-dev@firecloud.org" ],
            "name" : "Terra DataRepo"
            },
            "commit" : "73a852f36483818a9c89cc684448a360bc642fcd",
            "uuid" : "65b23fab-27af-4d47-8f88-46bb8e89df37",
            "executor" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "methodConfiguration" : "wfl-dev/sarscov2_illumina_full",
            "methodConfigurationVersion" : 2,
            "fromSource" : "importSnapshot",
            "name" : "Terra"
            },
            "version" : "0.8.0",
            "sink" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "entityType" : "flowcell",
            "fromOutputs" : {
            "submission_xml" : "submission_xml",
            "assembled_ids" : "assembled_ids",
            "num_failed_assembly" : "num_failed_assembly",
            "ivar_trim_stats_png" : "ivar_trim_stats_png",
            "read_counts_raw" : "read_counts_raw",
            "num_samples" : "num_samples",
            "vadr_outputs" : "vadr_outputs",
            "cleaned_reads_unaligned_bams" : "cleaned_reads_unaligned_bams",
            "demux_commonBarcodes" : "demux_commonBarcodes",
            "submission_zip" : "submission_zip",
            "cleaned_bams_tiny" : "cleaned_bams_tiny",
            "data_tables_out" : "data_tables_out",
            "ntc_rejected_batches" : "ntc_rejected_batches",
            "picard_metrics_alignment" : "picard_metrics_alignment",
            "failed_assembly_ids" : "failed_assembly_ids",
            "ivar_trim_stats_html" : "ivar_trim_stats_html",
            "assembly_stats_tsv" : "assembly_stats_tsv",
            "failed_annotation_ids" : "failed_annotation_ids",
            "run_date" : "run_date",
            "genbank_source_table" : "genbank_source_table",
            "num_read_files" : "num_read_files",
            "ntc_rejected_lanes" : "ntc_rejected_lanes",
            "primer_trimmed_read_count" : "primer_trimmed_read_count",
            "gisaid_fasta" : "gisaid_fasta",
            "num_submittable" : "num_submittable",
            "submit_ready" : "submit_ready",
            "passing_fasta" : "passing_fasta",
            "nextclade_auspice_json" : "nextclade_auspice_json",
            "read_counts_depleted" : "read_counts_depleted",
            "cleaned_bam_uris" : "cleaned_bam_uris",
            "num_assembled" : "num_assembled",
            "max_ntc_bases" : "max_ntc_bases",
            "genbank_fasta" : "genbank_fasta",
            "multiqc_report_cleaned" : "multiqc_report_cleaned",
            "num_failed_annotation" : "num_failed_annotation",
            "meta_by_filename_json" : "meta_by_filename_json",
            "primer_trimmed_read_percent" : "primer_trimmed_read_percent",
            "assembly_stats_final_tsv" : "assembly_stats_final_tsv",
            "demux_metrics" : "demux_metrics",
            "submittable_ids" : "submittable_ids",
            "sra_metadata" : "sra_metadata",
            "spikein_counts" : "spikein_counts",
            "raw_reads_unaligned_bams" : "raw_reads_unaligned_bams",
            "ivar_trim_stats_tsv" : "ivar_trim_stats_tsv",
            "picard_metrics_wgs" : "picard_metrics_wgs",
            "nextclade_all_json" : "nextclade_all_json",
            "multiqc_report_raw" : "multiqc_report_raw",
            "sequencing_reports" : "sequencing_reports",
            "demux_outlierBarcodes" : "demux_outlierBarcodes",
            "nextmeta_tsv" : "nextmeta_tsv",
            "assemblies_fasta" : "assemblies_fasta"
            },
            "identifier" : "run_id",
            "name" : "Terra Workspace"
            }
        } 
    ]
    ```

**GET /api/v1/workload?uuid={uuid}**

=== "Sample Request"

    ```
    curl 'http://localhost:8080/api/v1/workload?uuid=65b23fab-27af-4d47-8f88-46bb8e89df37' -H 'Authorization: Bearer '$(gcloud auth print-access-token)      -H 'Accept: application/json'
    ```

=== "Sample Response"

    ```
    [ 
        {   
            "watchers" : [ "tester@123.com" ],
            "labels" : [ "hornet:test", "project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy" ],
            "creator" : "ranthony@broadinstitute.org",
            "created" : "2021-06-15T13:25:07Z",
            "source" : {
            "dataset" : "4bb51d98-b4aa-4c72-b76a-1a96a2ee3057",
            "table" : "flowcells",
            "column" : "last_modified_date",
            "snapshotReaders" : [ "workflow-launcher-dev@firecloud.org" ],
            "name" : "Terra DataRepo"
            },
            "commit" : "73a852f36483818a9c89cc684448a360bc642fcd",
            "uuid" : "65b23fab-27af-4d47-8f88-46bb8e89df37",
            "executor" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "methodConfiguration" : "wfl-dev/sarscov2_illumina_full",
            "methodConfigurationVersion" : 2,
            "fromSource" : "importSnapshot",
            "name" : "Terra"
            },
            "version" : "0.8.0",
            "sink" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "entityType" : "flowcell",
            "fromOutputs" : {
            "submission_xml" : "submission_xml",
            "assembled_ids" : "assembled_ids",
            "num_failed_assembly" : "num_failed_assembly",
            "ivar_trim_stats_png" : "ivar_trim_stats_png",
            "read_counts_raw" : "read_counts_raw",
            "num_samples" : "num_samples",
            "vadr_outputs" : "vadr_outputs",
            "cleaned_reads_unaligned_bams" : "cleaned_reads_unaligned_bams",
            "demux_commonBarcodes" : "demux_commonBarcodes",
            "submission_zip" : "submission_zip",
            "cleaned_bams_tiny" : "cleaned_bams_tiny",
            "data_tables_out" : "data_tables_out",
            "ntc_rejected_batches" : "ntc_rejected_batches",
            "picard_metrics_alignment" : "picard_metrics_alignment",
            "failed_assembly_ids" : "failed_assembly_ids",
            "ivar_trim_stats_html" : "ivar_trim_stats_html",
            "assembly_stats_tsv" : "assembly_stats_tsv",
            "failed_annotation_ids" : "failed_annotation_ids",
            "run_date" : "run_date",
            "genbank_source_table" : "genbank_source_table",
            "num_read_files" : "num_read_files",
            "ntc_rejected_lanes" : "ntc_rejected_lanes",
            "primer_trimmed_read_count" : "primer_trimmed_read_count",
            "gisaid_fasta" : "gisaid_fasta",
            "num_submittable" : "num_submittable",
            "submit_ready" : "submit_ready",
            "passing_fasta" : "passing_fasta",
            "nextclade_auspice_json" : "nextclade_auspice_json",
            "read_counts_depleted" : "read_counts_depleted",
            "cleaned_bam_uris" : "cleaned_bam_uris",
            "num_assembled" : "num_assembled",
            "max_ntc_bases" : "max_ntc_bases",
            "genbank_fasta" : "genbank_fasta",
            "multiqc_report_cleaned" : "multiqc_report_cleaned",
            "num_failed_annotation" : "num_failed_annotation",
            "meta_by_filename_json" : "meta_by_filename_json",
            "primer_trimmed_read_percent" : "primer_trimmed_read_percent",
            "assembly_stats_final_tsv" : "assembly_stats_final_tsv",
            "demux_metrics" : "demux_metrics",
            "submittable_ids" : "submittable_ids",
            "sra_metadata" : "sra_metadata",
            "spikein_counts" : "spikein_counts",
            "raw_reads_unaligned_bams" : "raw_reads_unaligned_bams",
            "ivar_trim_stats_tsv" : "ivar_trim_stats_tsv",
            "picard_metrics_wgs" : "picard_metrics_wgs",
            "nextclade_all_json" : "nextclade_all_json",
            "multiqc_report_raw" : "multiqc_report_raw",
            "sequencing_reports" : "sequencing_reports",
            "demux_outlierBarcodes" : "demux_outlierBarcodes",
            "nextmeta_tsv" : "nextmeta_tsv",
            "assemblies_fasta" : "assemblies_fasta"
            },
            "identifier" : "run_id",
            "name" : "Terra Workspace"
            }
        } 
    ]
    ```

**GET /api/v1/workload/{uuid}/workflows**

=== "Sample Request"

    ```
    curl 'http://localhost:8080/api/v1/workload/ed6e8637-5bae-4602-8b7f-f5a1bbfd2406/workflows' -H 'Authorization: Bearer '$(gcloud auth print-access-token)      -H 'Accept: application/json'
    ```

**POST /api/v1/create**

=== "Sample Request"

    ```
    curl -X "POST" "http://localhost:8080/api/v1/create" \
         -H 'Authorization: Bearer '$(gcloud auth print-access-token) \
         -H 'Accept: application/json' \
         -H 'Content-Type: application/json' \
         -d '{
                "watchers": [
                    "tester@123.com"
                ],
                "labels": [
                    "hornet:test"
                ],
                "project": "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
                "pipeline": null,
                "source": {
                    "name": "Terra DataRepo",
                    "dataset": "4bb51d98-b4aa-4c72-b76a-1a96a2ee3057",
                    "table": "flowcells",
                    "column": "last_modified_date",
                    "snapshotReaders": [
                        "workflow-launcher-dev@firecloud.org"
                    ]
                },
                "executor": {
                    "workspace": "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
                    "methodConfiguration": "wfl-dev/sarscov2_illumina_full",
                    "methodConfigurationVersion": 2,
                    "fromSource": "importSnapshot",
                    "name": "Terra"
                },
                "sink": {
                    "name": "Terra Workspace",
                    "workspace": "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
                    "entityType": "flowcell",
                    "identifier": "run_id",
                    "skipValidation": "true",
                    "fromOutputs": {
                    "genbank_source_table": "genbank_source_table",
                    "demux_metrics": "demux_metrics",
                    "ntc_rejected_batches": "ntc_rejected_batches",
                    "assembly_stats_final_tsv": "assembly_stats_final_tsv",
                    "num_assembled": "num_assembled",
                    "picard_metrics_wgs": "picard_metrics_wgs",
                    "genbank_fasta": "genbank_fasta",
                    "ivar_trim_stats_tsv": "ivar_trim_stats_tsv",
                    "assembled_ids": "assembled_ids",
                    "read_counts_depleted": "read_counts_depleted",
                    "assembly_stats_tsv": "assembly_stats_tsv",
                    "raw_reads_unaligned_bams": "raw_reads_unaligned_bams",
                    "read_counts_raw": "read_counts_raw",
                    "demux_commonBarcodes": "demux_commonBarcodes",
                    "meta_by_filename_json": "meta_by_filename_json",
                    "submission_zip": "submission_zip",
                    "nextclade_auspice_json": "nextclade_auspice_json",
                    "data_tables_out": "data_tables_out",
                    "num_failed_annotation": "num_failed_annotation",
                    "num_submittable": "num_submittable",
                    "run_date": "run_date",
                    "cleaned_reads_unaligned_bams": "cleaned_reads_unaligned_bams",
                    "failed_annotation_ids": "failed_annotation_ids",
                    "demux_outlierBarcodes": "demux_outlierBarcodes",
                    "failed_assembly_ids": "failed_assembly_ids",
                    "cleaned_bam_uris": "cleaned_bam_uris",
                    "spikein_counts": "spikein_counts",
                    "ntc_rejected_lanes": "ntc_rejected_lanes",
                    "num_samples": "num_samples",
                    "picard_metrics_alignment": "picard_metrics_alignment",
                    "assemblies_fasta": "assemblies_fasta",
                    "num_read_files": "num_read_files",
                    "ivar_trim_stats_png": "ivar_trim_stats_png",
                    "ivar_trim_stats_html": "ivar_trim_stats_html",
                    "nextmeta_tsv": "nextmeta_tsv",
                    "primer_trimmed_read_percent": "primer_trimmed_read_percent",
                    "sequencing_reports": "sequencing_reports",
                    "sra_metadata": "sra_metadata",
                    "submit_ready": "submit_ready",
                    "primer_trimmed_read_count": "primer_trimmed_read_count",
                    "multiqc_report_cleaned": "multiqc_report_cleaned",
                    "max_ntc_bases": "max_ntc_bases",
                    "multiqc_report_raw": "multiqc_report_raw",
                    "cleaned_bams_tiny": "cleaned_bams_tiny",
                    "submittable_ids": "submittable_ids",
                    "gisaid_fasta": "gisaid_fasta",
                    "num_failed_assembly": "num_failed_assembly",
                    "passing_fasta": "passing_fasta",
                    "nextclade_all_json": "nextclade_all_json",
                    "submission_xml": "submission_xml",
                    "vadr_outputs": "vadr_outputs"
                    }
                }
            }'
    ```

=== "Sample Response"

    ```
    {
        "watchers" : [ 
            "tester@123.com" 
        ],
        "labels" : [ 
            "hornet:test", 
            "project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy" 
        ],
        "creator" : "ranthony@broadinstitute.org",
        "created" : "2021-06-15T18:50:52Z",
        "source" : {
            "dataset" : "4bb51d98-b4aa-4c72-b76a-1a96a2ee3057",
            "table" : "flowcells",
            "column" : "last_modified_date",
            "snapshotReaders" : [ 
                "workflow-launcher-dev@firecloud.org" 
            ],
            "name" : "Terra DataRepo"
        },
        "commit" : "73a852f36483818a9c89cc684448a360bc642fcd",
        "uuid" : "fb06bcf3-bc10-471b-a309-b2f99e4f5a67",
        "executor" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "methodConfiguration" : "wfl-dev/sarscov2_illumina_full",
            "methodConfigurationVersion" : 2,
            "fromSource" : "importSnapshot",
            "name" : "Terra"
        },
        "version" : "0.8.0",
        "sink" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "entityType" : "flowcell",
            "fromOutputs" : {
                "submission_xml" : "submission_xml",
                "assembled_ids" : "assembled_ids",
                "num_failed_assembly" : "num_failed_assembly",
                "ivar_trim_stats_png" : "ivar_trim_stats_png",
                "read_counts_raw" : "read_counts_raw",
                "num_samples" : "num_samples",
                "vadr_outputs" : "vadr_outputs",
                "cleaned_reads_unaligned_bams" : "cleaned_reads_unaligned_bams",
                "demux_commonBarcodes" : "demux_commonBarcodes",
                "submission_zip" : "submission_zip",
                "cleaned_bams_tiny" : "cleaned_bams_tiny",
                "data_tables_out" : "data_tables_out",
                "ntc_rejected_batches" : "ntc_rejected_batches",
                "picard_metrics_alignment" : "picard_metrics_alignment",
                "failed_assembly_ids" : "failed_assembly_ids",
                "ivar_trim_stats_html" : "ivar_trim_stats_html",
                "assembly_stats_tsv" : "assembly_stats_tsv",
                "failed_annotation_ids" : "failed_annotation_ids",
                "run_date" : "run_date",
                "genbank_source_table" : "genbank_source_table",
                "num_read_files" : "num_read_files",
                "ntc_rejected_lanes" : "ntc_rejected_lanes",
                "primer_trimmed_read_count" : "primer_trimmed_read_count",
                "gisaid_fasta" : "gisaid_fasta",
                "num_submittable" : "num_submittable",
                "submit_ready" : "submit_ready",
                "passing_fasta" : "passing_fasta",
                "nextclade_auspice_json" : "nextclade_auspice_json",
                "read_counts_depleted" : "read_counts_depleted",
                "cleaned_bam_uris" : "cleaned_bam_uris",
                "num_assembled" : "num_assembled",
                "max_ntc_bases" : "max_ntc_bases",
                "genbank_fasta" : "genbank_fasta",
                "multiqc_report_cleaned" : "multiqc_report_cleaned",
                "num_failed_annotation" : "num_failed_annotation",
                "meta_by_filename_json" : "meta_by_filename_json",
                "primer_trimmed_read_percent" : "primer_trimmed_read_percent",
                "assembly_stats_final_tsv" : "assembly_stats_final_tsv",
                "demux_metrics" : "demux_metrics",
                "submittable_ids" : "submittable_ids",
                "sra_metadata" : "sra_metadata",
                "spikein_counts" : "spikein_counts",
                "raw_reads_unaligned_bams" : "raw_reads_unaligned_bams",
                "ivar_trim_stats_tsv" : "ivar_trim_stats_tsv",
                "picard_metrics_wgs" : "picard_metrics_wgs",
                "nextclade_all_json" : "nextclade_all_json",
                "multiqc_report_raw" : "multiqc_report_raw",
                "sequencing_reports" : "sequencing_reports",
                "demux_outlierBarcodes" : "demux_outlierBarcodes",
                "nextmeta_tsv" : "nextmeta_tsv",
                "assemblies_fasta" : "assemblies_fasta"
            },
            "identifier" : "run_id",
            "name" : "Terra Workspace"
        }
    }
    ```

**POST /api/v1/start**

=== "Sample Request"

    ```
    curl -X POST 'http://localhost:8080/api/v1/start' \
         -H 'Authorization: Bearer '$(gcloud auth print-access-token) \
         -H 'Accept: application/json' \
         -H 'Content-Type: application/json' \
         -d $'{ "uuid": "fb06bcf3-bc10-471b-a309-b2f99e4f5a67" }'
    ```

=== "Sample Response"

    ```
    {
        "started" : "2021-06-15T18:54:57Z",
        "watchers" : [ 
            "tester@123.com" 
        ],
        "labels" : [ 
            "hornet:test", 
            "project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy" 
        ],
        "creator" : "ranthony@broadinstitute.org",
        "updated" : "2021-06-15T18:54:57Z",
        "created" : "2021-06-15T18:50:52Z",
        "source" : {
            "dataset" : "4bb51d98-b4aa-4c72-b76a-1a96a2ee3057",
            "table" : "flowcells",
            "column" : "last_modified_date",
            "snapshotReaders" : [ 
                "workflow-launcher-dev@firecloud.org" 
            ],
            "name" : "Terra DataRepo"
        },
        "commit" : "73a852f36483818a9c89cc684448a360bc642fcd",
        "uuid" : "fb06bcf3-bc10-471b-a309-b2f99e4f5a67",
        "executor" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "methodConfiguration" : "wfl-dev/sarscov2_illumina_full",
            "methodConfigurationVersion" : 2,
            "fromSource" : "importSnapshot",
            "name" : "Terra"
        },
        "version" : "0.8.0",
        "sink" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "entityType" : "flowcell",
            "fromOutputs" : {
                "submission_xml" : "submission_xml",
                "assembled_ids" : "assembled_ids",
                "num_failed_assembly" : "num_failed_assembly",
                "ivar_trim_stats_png" : "ivar_trim_stats_png",
                "read_counts_raw" : "read_counts_raw",
                "num_samples" : "num_samples",
                "vadr_outputs" : "vadr_outputs",
                "cleaned_reads_unaligned_bams" : "cleaned_reads_unaligned_bams",
                "demux_commonBarcodes" : "demux_commonBarcodes",
                "submission_zip" : "submission_zip",
                "cleaned_bams_tiny" : "cleaned_bams_tiny",
                "data_tables_out" : "data_tables_out",
                "ntc_rejected_batches" : "ntc_rejected_batches",
                "picard_metrics_alignment" : "picard_metrics_alignment",
                "failed_assembly_ids" : "failed_assembly_ids",
                "ivar_trim_stats_html" : "ivar_trim_stats_html",
                "assembly_stats_tsv" : "assembly_stats_tsv",
                "failed_annotation_ids" : "failed_annotation_ids",
                "run_date" : "run_date",
                "genbank_source_table" : "genbank_source_table",
                "num_read_files" : "num_read_files",
                "ntc_rejected_lanes" : "ntc_rejected_lanes",
                "primer_trimmed_read_count" : "primer_trimmed_read_count",
                "gisaid_fasta" : "gisaid_fasta",
                "num_submittable" : "num_submittable",
                "submit_ready" : "submit_ready",
                "passing_fasta" : "passing_fasta",
                "nextclade_auspice_json" : "nextclade_auspice_json",
                "read_counts_depleted" : "read_counts_depleted",
                "cleaned_bam_uris" : "cleaned_bam_uris",
                "num_assembled" : "num_assembled",
                "max_ntc_bases" : "max_ntc_bases",
                "genbank_fasta" : "genbank_fasta",
                "multiqc_report_cleaned" : "multiqc_report_cleaned",
                "num_failed_annotation" : "num_failed_annotation",
                "meta_by_filename_json" : "meta_by_filename_json",
                "primer_trimmed_read_percent" : "primer_trimmed_read_percent",
                "assembly_stats_final_tsv" : "assembly_stats_final_tsv",
                "demux_metrics" : "demux_metrics",
                "submittable_ids" : "submittable_ids",
                "sra_metadata" : "sra_metadata",
                "spikein_counts" : "spikein_counts",
                "raw_reads_unaligned_bams" : "raw_reads_unaligned_bams",
                "ivar_trim_stats_tsv" : "ivar_trim_stats_tsv",
                "picard_metrics_wgs" : "picard_metrics_wgs",
                "nextclade_all_json" : "nextclade_all_json",
                "multiqc_report_raw" : "multiqc_report_raw",
                "sequencing_reports" : "sequencing_reports",
                "demux_outlierBarcodes" : "demux_outlierBarcodes",
                "nextmeta_tsv" : "nextmeta_tsv",
                "assemblies_fasta" : "assemblies_fasta"
            },
            "identifier" : "run_id",
            "name" : "Terra Workspace"
            }
        }
    ```

**POST /api/v1/stop**

=== "Sample Request"

    ```
    curl -X POST 'http://localhost:8080/api/v1/start' \
         -H 'Authorization: Bearer '$(gcloud auth print-access-token) \
         -H 'Accept: application/json' \
         -H 'Content-Type: application/json' \
         -d $'{ "uuid": "fb06bcf3-bc10-471b-a309-b2f99e4f5a67" }'
    ```

=== "Sample Response"

    ```
    {
        "started" : "2021-06-15T18:54:57Z",
        "watchers" : [ 
            "tester@123.com" 
        ],
        "labels" : [ 
            "hornet:test", 
            "project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy" 
        ],
        "creator" : "ranthony@broadinstitute.org",
        "updated" : "2021-06-15T18:59:28Z",
        "created" : "2021-06-15T18:50:52Z",
        "source" : {
            "dataset" : "4bb51d98-b4aa-4c72-b76a-1a96a2ee3057",
            "table" : "flowcells",
            "column" : "last_modified_date",
            "snapshotReaders" : [ 
                "workflow-launcher-dev@firecloud.org" 
            ],
            "name" : "Terra DataRepo"
        },
        "stopped" : "2021-06-15T18:59:28Z",
        "commit" : "73a852f36483818a9c89cc684448a360bc642fcd",
        "uuid" : "fb06bcf3-bc10-471b-a309-b2f99e4f5a67",
        "executor" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "methodConfiguration" : "wfl-dev/sarscov2_illumina_full",
            "methodConfigurationVersion" : 2,
            "fromSource" : "importSnapshot",
            "name" : "Terra"
        },
        "version" : "0.8.0",
        "sink" : {
            "workspace" : "wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy",
            "entityType" : "flowcell",
            "fromOutputs" : {
                "submission_xml" : "submission_xml",
                "assembled_ids" : "assembled_ids",
                "num_failed_assembly" : "num_failed_assembly",
                "ivar_trim_stats_png" : "ivar_trim_stats_png",
                "read_counts_raw" : "read_counts_raw",
                "num_samples" : "num_samples",
                "vadr_outputs" : "vadr_outputs",
                "cleaned_reads_unaligned_bams" : "cleaned_reads_unaligned_bams",
                "demux_commonBarcodes" : "demux_commonBarcodes",
                "submission_zip" : "submission_zip",
                "cleaned_bams_tiny" : "cleaned_bams_tiny",
                "data_tables_out" : "data_tables_out",
                "ntc_rejected_batches" : "ntc_rejected_batches",
                "picard_metrics_alignment" : "picard_metrics_alignment",
                "failed_assembly_ids" : "failed_assembly_ids",
                "ivar_trim_stats_html" : "ivar_trim_stats_html",
                "assembly_stats_tsv" : "assembly_stats_tsv",
                "failed_annotation_ids" : "failed_annotation_ids",
                "run_date" : "run_date",
                "genbank_source_table" : "genbank_source_table",
                "num_read_files" : "num_read_files",
                "ntc_rejected_lanes" : "ntc_rejected_lanes",
                "primer_trimmed_read_count" : "primer_trimmed_read_count",
                "gisaid_fasta" : "gisaid_fasta",
                "num_submittable" : "num_submittable",
                "submit_ready" : "submit_ready",
                "passing_fasta" : "passing_fasta",
                "nextclade_auspice_json" : "nextclade_auspice_json",
                "read_counts_depleted" : "read_counts_depleted",
                "cleaned_bam_uris" : "cleaned_bam_uris",
                "num_assembled" : "num_assembled",
                "max_ntc_bases" : "max_ntc_bases",
                "genbank_fasta" : "genbank_fasta",
                "multiqc_report_cleaned" : "multiqc_report_cleaned",
                "num_failed_annotation" : "num_failed_annotation",
                "meta_by_filename_json" : "meta_by_filename_json",
                "primer_trimmed_read_percent" : "primer_trimmed_read_percent",
                "assembly_stats_final_tsv" : "assembly_stats_final_tsv",
                "demux_metrics" : "demux_metrics",
                "submittable_ids" : "submittable_ids",
                "sra_metadata" : "sra_metadata",
                "spikein_counts" : "spikein_counts",
                "raw_reads_unaligned_bams" : "raw_reads_unaligned_bams",
                "ivar_trim_stats_tsv" : "ivar_trim_stats_tsv",
                "picard_metrics_wgs" : "picard_metrics_wgs",
                "nextclade_all_json" : "nextclade_all_json",
                "multiqc_report_raw" : "multiqc_report_raw",
                "sequencing_reports" : "sequencing_reports",
                "demux_outlierBarcodes" : "demux_outlierBarcodes",
                "nextmeta_tsv" : "nextmeta_tsv",
                "assemblies_fasta" : "assemblies_fasta"
            },
            "identifier" : "run_id",
            "name" : "Terra Workspace"
            }
        }
    ```

### Workload Model

WFL designed the following workload model in order to support the above API and workload submission mechanism.

Initially, it has the following schema:

```
                                    List of relations
 Schema |              Name              |   Type   |  Owner   |    Size    | Description
--------+--------------------------------+----------+----------+------------+-------------
 public | databasechangelog              | table    | foo      | 16 kB      |
 public | databasechangeloglock          | table    | foo      | 8192 bytes |
 public | workload                       | table    | foo      | 16 kB      |
 public | workload_id_seq                | sequence | foo      | 8192 bytes |
```

the `workload` table looks like:

```
id | commit | created | creator | executor | finished | input | items | output | pipeline | project | release | started | uuid | version | wdl | stopped | source_type | source_items | executor_type | executor_items | sink_type | sink_items | updated | labels | watchers 
---+--------+---------+---------+----------+----------+-------+-------+--------+----------+---------+---------+---------+------+---------+-----+---------+-------------+--------------+---------------+----------------+-----------+------------+---------+--------+---------
(0 rows)
```

When a workload is created it is added to the table above, and several tables are created: 

```
id | commit                                   | created                       | creator                     | executor | finished | input | items | output | pipeline | project                                             | release | started | uuid                                 | version | wdl | stopped | source_type         | source_items | executor_type | executor_items | sink_type          | sink_items | updated | labels                                                                      | watchers 
---+------------------------------------------+-------------------------------+-----------------------------+----------+----------+-------+-------+--------+----------+-----------------------------------------------------+---------+---------+--------------------------------------+---------+-----+---------+---------------------+--------------+---------------+----------------+--------------------+------------+---------+-----------------------------------------------------------------------------+-----------------
 1 | 73a852f36483818a9c89cc684448a360bc642fcd | 2021-06-15 15:09:46.921657-04 | ranthony@broadinstitute.org |          |          |       |       |        |          | wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy |         |         | 43462a9e-a642-4693-9b44-e1874fe380a6 | 0.8.0   |     |         | TerraDataRepoSource | 1            | TerraExecutor | 1              | TerraWorkspaceSink | 1          |         | {hornet:test,"project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy"} | {tester@123.com}
(1 row)
```

After start, the database will look like this: 
```
id | commit                                   | created                       | creator                     | executor | finished | input | items | output | pipeline | project                                             | release | started                              | uuid                                 | version | wdl | stopped | source_type         | source_items | executor_type | executor_items | sink_type          | sink_items | updated                              | labels                                                                      | watchers 
---+------------------------------------------+-------------------------------+-----------------------------+----------+----------+-------+-------+--------+----------+-----------------------------------------------------+---------+--------------------------------------+--------------------------------------+---------+-----+---------+---------------------+--------------+---------------+----------------+--------------------+------------+--------------------------------------+-----------------------------------------------------------------------------+-----------------
 1 | 73a852f36483818a9c89cc684448a360bc642fcd | 2021-06-15 15:09:46.921657-04 | ranthony@broadinstitute.org |          |          |       |       |        |          | wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy |         | 2021-06-15 15:16:57.210555-04        | 43462a9e-a642-4693-9b44-e1874fe380a6 | 0.8.0   |     |         | TerraDataRepoSource | 1            | TerraExecutor | 1              | TerraWorkspaceSink | 1          | 2021-06-15 15:16:57.210555-04        | {hornet:test,"project:wfl-dev/CDC_Viral_Sequencing _ranthony_bashing_copy"} | {tester@123.com}
(1 row)
```

Meanwhile other tables will be created for the source, the executor and the sink according to the type of each invoked by the workload request:
```
Schema |                 Name                 |   Type   |  Owner   
-------+--------------------------------------+----------+----------
public | databasechangelog                    | table    | ranthony
public | databasechangeloglock                | table    | ranthony
public | terradatareposource                  | table    | ranthony
public | terradatareposource_000000001        | table    | ranthony
public | terradatareposource_000000001_id_seq | sequence | ranthony
public | terradatareposource_id_seq           | sequence | ranthony
public | terraexecutor                        | table    | ranthony
public | terraexecutor_000000001              | table    | ranthony
public | terraexecutor_000000001_id_seq       | sequence | ranthony
public | terraexecutor_id_seq                 | sequence | ranthony
public | terraworkspacesink                   | table    | ranthony
public | terraworkspacesink_000000001         | table    | ranthony
public | terraworkspacesink_000000001_id_seq  | sequence | ranthony
```