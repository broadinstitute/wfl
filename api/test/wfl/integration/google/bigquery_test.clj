...
(def ^:private dr-view-content
  {:kind "bigquery#queryResponse"
   ...
   :jobReference {:projectId "broad-jade-dev-data"
                  :jobId     "job_Zd6Ld4uCl8kmuFkiGKsPdk5OnBNP"
                  :location  "US"}
   :totalRows "1"
...
(deftest test-query-table-sync
  (let [response (bigquery/query-table-sync google-project dr-dataset dr-view)]
    (is (= "bigquery#queryResponse" (:kind response)))))
...
(deftest test-dump-table->tsv
  (let [terra-table-name "test-name"
    ...
    (is (= (format "entity:%s_id" terra-table-name)
        ...
        "The result TSV header is not properly formatted!")))
