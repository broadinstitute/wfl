...
(defn submit-arrays-workflow-terra
  "Submit one workflow to terra and return the Cromwell UUID."
  [terra-url workspace method-configuration-name
   method-configuration-namespace entity-type entity-name]
  (let [submission-id (terra/create-submission terra-url workspace method-configuration-name
                                               method-configuration-namespace entity-type entity-name)]
    (->>
      (util/timeout 30000 #(terra/wait-for-workflow-ids terra-url workspace submission-id))
      (first)
      :workflowId)))
...
(defn start-arrays-workload!
  ...
  (let [now (OffsetDateTime/now)
        terra-url "https://firecloud-orchestration.dsde-dev.broadinstitute.org"
    ...
    (letfn [(submit! [{:keys [id] :as workflow}]
              (let [inputs           (:inputs workflow)
                ...
                [id (submit-arrays-workflow-terra terra-url project method-configuration-name
                                                      method-configuration-namespace entity-type entity-name)]))
            (update! [tx [id uuid]]
              (when uuid
